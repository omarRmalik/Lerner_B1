...
def myrange_gen(x, y, increment=1):
  current_val = x
  yield x
  while current_val < (y - increment):
    current_val = current_val + increment
...
def myrange3(*args):
  if not args:
    ...
    non_none_args =[one_arg for one_arg in args
                    if one_arg is not None]
    if len(non_none_args) == 1:
      return myrange_gen(x = 0, y = non_none_args[0], increment = 1)
    elif len(non_none_args) == 2 and non_none_args[0] < non_none_args[1]:
      return myrange_gen(x=non_none_args[0], y=non_none_args[1], increment=1)
    elif len(non_none_args) == 3:
      return myrange_gen(x = non_none_args[0], y = non_none_args[1], increment = non_none_args[2])
    else:
      return [ ]